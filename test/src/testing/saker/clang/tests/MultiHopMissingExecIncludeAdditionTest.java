...
@SakerTest
public class MultiHopMissingExecIncludeAdditionTest extends ClangTestCase {
	private static final SakerPath PATH_MAINC_OBJ = PATH_BUILD_DIRECTORY
			.resolve("saker.clang.compile/default/main.c.o");

	@Override
	protected void runTestImpl() throws Throwable {
		SakerPath firstheaderpath = PATH_WORKING_DIRECTORY.resolve("include/first.h");
		SakerPath secondheaderpath = PATH_WORKING_DIRECTORY.resolve("include/second.h");
		...
		files.putFile(firstheaderpath, "#include <second.h>\n10".getBytes());
		assertException(Exception.class, () -> runScriptTask("build"));
		assertNotEmpty(getMetric().getRunTaskIdDeltas());

		//no changes, so nothing should be reinvoked
		assertException(Exception.class, () -> runScriptTask("build"));
		assertEmpty(getMetric().getRunTaskIdDeltas());

		files.putFile(secondheaderpath, "20".getBytes());
		runScriptTask("build");
		assertEquals(files.getAllBytes(PATH_MAINC_OBJ).toString(), compile(LANG_C, TARGET_DEFAULT, 20, 10, 456));
	}
